...
const copyToClipBoard = (text: string) => {
  if (text) {
    ...
    notify({
      group: 'generic', type: 'modest', title: 'Copied!', text: `Text in buffer: ${text}`
    }, 3000)
  }
}
const swapLanguages = () => {
...
const getData = async () => {
  ...
  await fetch('https://nw6usm5uha.us.aircode.run/demo', {
    ...
    if (error) notify({
      group: 'generic', type: 'error', title: 'Server Error 502', text: 'Cant\'t get result from deepl translation'
    }, 4000)
    isLoading.value = false
  })
}
...
<template>
  <main class="dark:bg-main-dark bg-main text-white h-screen relative overflow-hidden">
    <section class="h-fit p-2 mx-3 mt-4 rounded-lg">
      <div class="flex w-full py-2 bg-secondary mb-2 dark:bg-secondary-dark justify-between rounded-md relative">
        <Listbox
          v-model="sourceLang"
          class="w-1/2"
        >
          <div class="relative">
            ...
            >
              <ListboxOptions
                class="absolute mt-1 left-0 max-h-60 w-full overflow-auto rounded-md  shadow-lg backdrop-blur-sm border border-zinc-700 dark:bg-black/50 bg-white/50 py-1 text-base ring-1 ring-black/5 focus:outline-none sm:text-sm"
              >
                <ListboxOption
                  v-for="item in languages.filter(d => d.code !== targetLang.code)"
                  v-slot="{ active, selected }"
                  :key="item.title"
                  :value="item"
                  :disabled="item.code === targetLang.code"
                  as="template"
                >
                  <li
                    :class="[
                      active ? 'bg-zinc-400/80 dark:bg-secondary/10 dark:text-zinc-300 text-zinc-900' : 'text-zinc-900 dark:text-zinc-600',
                      'relative cursor-default select-none py-2 px-4 duration-150 transition-all ease-linear',
                  ...
                  >
                    ...
                    >{{ item.title }}</span>
                    <span
                      v-if="item.code === targetLang.code"
        ...
        <button
          class="bg-secondary dark:bg-secondary-dark/90 dark:hover:bg-black/20 border border-zinc-600 items-center flex justify-center rounded-md px-2.5"
          @click="swapLanguages()"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        >
          <div class="relative">
            ...
            >
              <ListboxOptions
                class="absolute left-0 mt-1 max-h-60 w-full overflow-auto rounded-md  shadow-lg backdrop-blur-sm border border-zinc-700 dark:bg-black/50 bg-white/50 py-1 text-base ring-1 ring-black/5 focus:outline-none sm:text-sm"
              >
                <ListboxOption
                  v-for="item in languages.filter(d => d.code !== sourceLang.code)"
                  v-slot="{ active, selected }"
                  :key="item.title"
                  :value="item"
                  :disabled="item.code === sourceLang.code"
                  as="template"
                >
                  <li
                    :class="[
                  ...
                  >
                    ...
                    >{{ item.title }}</span>
                    <span
                      v-if="item.code === sourceLang.code"
                      class="absolute inset-y-0 left-0 flex items-center pl-3 text-amber-600"
                    >
                      <svg
                        class="h-4 w-4 text-green-700"
        ...
        <div class="flex flex-col px-2 py-1 items-start">
          ...
          <div class="flex w-full gap-x-3">
            <button
              ...
              @click="copyToClipBoard(formData)"
            >
              <svg
                v-if="!isCopied"
                xmlns="http://www.w3.org/2000/svg"
      ...
      >
        <svg
          v-if="isLoading"
          xmlns="http://www.w3.org/2000/svg"
          class="h-6 w-6 text-zinc-600 fill-current"
          viewBox="0 0 24 24"
        >
          <g
            fill="none"
    ...
    <Transition name="slide-fade">
      <section
        ...
        class="h-fit p-2 mx-3 mt-4 rounded-lg"
      >
        <h3 class="font-semibold text-black dark:text-white text-base mb-2">
          Translation to <span
            class="text-accent font-bold dark:text-accent-dark"
          >{{ languages.filter(lang => lang.code === targetLang.code )[0].title }}</span>
        </h3>
        <div class="bg-secondary dark:bg-secondary-dark rounded-lg px-3 py-4 mb-4 flex items-center justify-between">
          ...
          <button
            ...
            @click="copyToClipBoard(translationData.data)"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-5 w-5"
        ...
        <template v-if="translationData.alternatives && translationData.alternatives.length > 0">
          ...
          >
            <span
              v-for="(item, k) in translationData.alternatives"
              :key="k + item"
              class="text-zinc-500/90 dark:text-zinc-600"
            >{{ `${k}): ` + item }}</span>
          </div>
        </template>
      </section>
...
<style>
.listbox-button {
    @apply relative items-center cursor-default rounded-lg dark:text-white text-black py-2 pl-3 pr-10 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-accent dark:focus-visible:ring-accent-dark sm:text-sm w-full max-w-[110px]
}
